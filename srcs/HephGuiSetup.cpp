...
static void s_callbackWindowSize(GLFWwindow* window, int width, int height) {
  HephGui *olivePtr = static_cast<HephGui*>(glfwGetWindowUserPointer(window));
...
void  HephGui::setupCallbackForWindow(GLFWwindow *window) {
...
HephResult	HephGui::hephaestusSetup() {
...
HephResult	HephGui::createHephSwapchain() {
...
HephResult	HephGui::createRenderPass() {
